...
# Define initial conditions and define the problem
begin
	# f = (x, y) -> y ≤ 1.0 ? 50.0 : 0.0 # Ternary Operator
	ϵ = 0.05
	S = 10
	f = (x, y) -> S * 1 / (ϵ^2 * π) * exp(-1 / (ϵ^2) * ((x-1)^2 + (y-1)^2))
...
begin
	Plot = Figure(fontsize=38)
	for (i, j) in zip(1:3, (1, 6, 11))
	    ax = Axis(Plot[1, i], width=800, height=800,
	        xlabel="x", ylabel="y",
	        title="t = $(sol.t[j])",
	        titlealign=:center)
